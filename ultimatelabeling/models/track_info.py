...
class Detection:
    def __init__(self, class_id=0, action_id=0, track_id=0, polygon=Polygon(), bbox=Bbox(), keypoints=Keypoints()):
    ...
    @staticmethod
    def from_json(data):
        return Detection(data["class_id"], data["action_id"], data["track_id"], 
    ...
    def to_dict(self):
        return {"track_id": self.track_id, "class_id": self.class_id, "action_id": self.action_id, **self.bbox.to_dict(),
                "polygon": self.polygon.to_str(), "kp": self.keypoints.to_str()}

    @staticmethod
    def from_df(row):
        bbox = Bbox(row.x, row.y, row.w, row.h)
        return Detection(row.class_id, row.action_id, row.track_id, Polygon.from_str(row.polygon), bbox, Keypoints.from_str(row.kp))

    def copy(self):
        return Detection(self.class_id, self.action_id, self.track_id, self.polygon.copy(), self.bbox.copy(), self.keypoints.copy())

    def __repr__(self):
        return "Detection(class_id={}, action_id={}, track_id={}, bbox={}, polygon={}, keypoints={})".format(self.class_id, self.action_id,
                                                                                                             self.track_id, self.bbox, self.polygon, self.keypoints)
...
class TrackInfo:
    ...
    def load_info(self):
        ...
        with open(json_file, "r") as f:
            ...
            self.action_names = {int(k): v for k, v in json.loads(data["action_names"]).items()}
            self.class_names = {int(k): v for k, v in json.loads(data["class_names"]).items()}

    @staticmethod
    def df_from_csv(file_name):
        if not os.path.exists(file_name):
            return pd.DataFrame(columns=["track_id", "class_id", "action_id", "x", "y", "w", "h", "polygon", "kp"])

        return pd.read_csv(file_name, header=None, names=["track_id", "class_id", "action_id", "x", "y", "w", "h", "polygon", "kp"],
                           na_filter=False, sep=" ")
    ...
    def to_df(self, file_names):
        df = pd.DataFrame(columns=["frame", "class_id", "action_id", "track_id", "xc", "yc", "w", "h", "infer", "polygon", "kp"])
        ...
        for i, file_name in enumerate(file_names):
            ...
            df_frame = df_frame[["frame", "class_id", "action_id", "track_id", "xc", "yc", "w", "h", "infer", "polygon", "kp"]]
    ...
    def from_df_all(self, df, file_names):
        ...
        df[["class_id", "action_id", "track_id"]] = df[["class_id", "action_id", "track_id"]].astype(int)
        ...
        for i, file_name in enumerate(file_names):
            ...
            df_frame = df_frame[["track_id", "class_id", "action_id", "x", "y", "w", "h", "polygon", "kp"]]
    ...
    def write_info(self):
        ...
        data = {
            ...
            "action_names": json.dumps(self.action_names)
        }
    ...
    def write_detections(self, file_name, detections=None):
        ...
        df = pd.DataFrame(columns=["track_id", "class_id", "action_id", "x", "y", "w", "h", "polygon", "kp"])
        if len(detections) > 0:
    ...
    def modify_action_id(self, track_id, action_id, file_name):
        """
        Modifies action id with specific track_id from detections file
        Returns true if at least one modification was done
        """
        ...
        df.loc[df.track_id == track_id, "action_id"] = action_id
